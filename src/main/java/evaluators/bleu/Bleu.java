...
public class Bleu {
	...
	public List<String> ngrams(String[] words, int n) {
	...
	private double translate(String translation, String reference, int n) {
	...
	private List<String> clipNgrams(List<String> translatedNGrams, String reference) {
		...
		for(String gram : translatedNGrams) {
			if(!repeated.containsKey(gram)) { repeated.put(gram, 1); }
			else { repeated.put(gram, repeated.get(gram)+1); }
		}
		for(String word : repeated.keySet()) {
			int count = count(reference, word);
			int wordCount = repeated.get(word);
			if(wordCount >= count) {
				for(int i = 0; i < wordCount - count; i++) { translatedNGrams.remove(word); }
			}
		}
