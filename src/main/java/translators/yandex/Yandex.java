...
public class Yandex {
    ...
    public Yandex(TextFileWriter textFileWriter, TextFileReader textFileReader, Bleu bleu, Meteor meteor) {
        this.textFileReader = textFileReader;
        this.textFileWriter = textFileWriter;
        this.bleu = bleu;
        this.meteor = meteor;
        this.yandexApiReader = new YandexApiReader();
    }
    ...
    public void translate(int lines) throws Exception {
        ...
        while(write <= lines) {
            if (text.get(read).length() > 5) {
                textFileWriter.write(yandexApiReader.read(source, target, text.get(read)));
                write++;
            }
            read++;
        }
